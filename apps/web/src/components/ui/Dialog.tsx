...
export function DialogHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn('p-4 border-b border-neutral-800 flex items-center justify-between', className)}
      {...props}
    />
  );
}
...
export function DialogClose({ children, className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button
      ...
      onClick={() => ctx?.setOpen(false)}
      className={cn(
        'h-8 w-8 inline-flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-800 hover:text-neutral-100',
        className,
      )}
      {...props}
    >
